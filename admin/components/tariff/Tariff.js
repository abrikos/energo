...
import TariffCategoryList from './TariffCategoryList';
...
export default class Tariff extends React.Component {
	...
	@action switchTab = record => {
		this.tab = record.id;
		this.props.history.push(`/admin/tariff_tabs/${record.id}/list`);
			...
			<div>
				...
				{this.records.map(record => (
					<React.Fragment key={`route-${record.id}`}>
						<Route
							exact
							path={`/admin/tariff_tabs/${record.id}/list`}
							component={() => <TariffCategoryList category={record} />}
						/>
						<Route
							path={`/admin/tariff_tabs/${record.id}/edit/:id`}
							component={() => <TariffCategory category={record} />}
						/>
					</React.Fragment>
				))}
			</div>
