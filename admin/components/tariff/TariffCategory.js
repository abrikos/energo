...
@withRouter
@observer
export default class TariffCategory extends React.Component {
	@observable record;
	@observable files;
	@observable id;
	...
	@action onSave = async record => {
		this.id && this.props.history.push(`/admin/tariff_tabs/${this.props.category.id}/list`);
	...
	render() {
		...
		const onDelete = () => {
			this.props.history.push(`/admin/tariff_tabs/${this.props.category.id}/list`);
		};
