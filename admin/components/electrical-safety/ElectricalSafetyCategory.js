...
@withRouter
@observer
export default class ElectricalSafetyCategory extends React.Component {
	@observable record;
	@observable files;
	@observable id;
	...
	init = async () => {
		...
		if (!isNaN(id)) {
			...
			record.electricalsafetyId = categoryId;
	...
	@action onSave = async record => {
		this.id && this.props.history.push(`/admin/electricalsafety/${this.props.category.id}/list`);
	...
	render() {
		...
		const onDelete = () => {
			this.props.history.push(`/admin/electricalsafety/${this.props.category.id}/list`);
		};
