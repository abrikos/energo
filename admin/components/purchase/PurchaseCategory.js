...
@withRouter
@observer
export default class PurchaseCategory extends React.Component {
	@observable record;
	@observable files;
	@observable id;
	...
	@action onSave = async record => {
		this.id && this.props.history.push(`/admin/purchase_tabs/${this.props.category.id}/list`);
	...
	render() {
		...
		const onDelete = () => {
			this.props.history.push(`/admin/purchase_tabs/${this.props.category.id}/list`);
		};
