...
import PurchaseCategoryList from './PurchaseCategoryList';
...
export default class Purchase extends React.Component {
	...
	@action switchTab = record => {
		this.tab = record.id;
		this.props.history.push(`/admin/purchase_tabs/${record.id}/list`);
			...
			<div>
				...
				{this.records.map(record => (
					<React.Fragment key={`route-${record.id}`}>
						<Route
							exact
							path={`/admin/purchase_tabs/${record.id}/list`}
							component={() => <PurchaseCategoryList category={record} />}
						/>
						<Route
							path={`/admin/purchase_tabs/${record.id}/edit/:id`}
							component={() => <PurchaseCategory category={record} />}
						/>
					</React.Fragment>
				))}
			</div>
