...
@withRouter
@observer
export default class ShareholderEdit extends React.Component {
	@observable record;
	@observable id;
	...
	init = async () => {
		...
		let record;
		if (!isNaN(id)) {
			record = await store.model.Shareholder.findById(id);
			this.id = id;
		} else {
			record = new store.model.Shareholder();
		}
		this.record = record;
	...
	save = async record => {
		...
		await record.save();
		// console.log('saved');
		this.props.history.push('/admin/shareholders');
	};
