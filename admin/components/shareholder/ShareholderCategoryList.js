...
@withRouter
@observer
export default class ShareholderCategoryList extends React.Component {
	@observable records;
	...
	@action init = async () => {
		const records = await store.model.CategoryRecord.find({
			where: {
				shareholderId: this.props.category.id,
			},
		});
	...
	render() {
		...
		const onRowClick = record => {
			this.props.history.push(`/admin/shareholder/${this.props.category.id}/edit/${record.id}`);
		};
			...
			<div>
				<div className="filters">
					<Button
						...
						onClick={() => this.props.history.push(`/admin/shareholder/${this.props.category.id}/edit/create`)}
					/>
				</div>
