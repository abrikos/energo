...
import ShareholderCategoryList from './ShareholderCategoryList';
...
export default class Shareholder extends React.Component {
	...
	@action switchTab = record => {
		this.tab = record.id;
		this.props.history.push(`/admin/shareholder/${record.id}/list`);
			...
			<div>
				...
				{this.records.map(record => (
					<React.Fragment key={`route-${record.id}`}>
						<Route
							exact
							path={`/admin/shareholder/${record.id}/list`}
							component={() => <ShareholderCategoryList category={record} />}
						/>
						<Route
							path={`/admin/shareholder/${record.id}/edit/:id`}
							component={() => <ShareholderCategory category={record} />}
						/>
					</React.Fragment>
				))}
			</div>
