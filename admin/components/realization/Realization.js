...
import RealizationCategory from './RealizationCategory';
import RealizationCategoryList from './RealizationCategoryList';
...
@withRouter
@observer
export default class Realization extends React.Component {
	@observable records;
	@observable tab;
	...
	@action switchTab = record => {
		this.tab = record.id;
		this.props.history.push(`/admin/realization_tabs/${record.id}/list`);
			...
			<div>
				...
				{this.records.map(record => (
					<React.Fragment key={`route-${record.id}`}>
						<Route
							exact
							path={`/admin/realization_tabs/${record.id}/list`}
							component={() => <RealizationCategoryList category={record} />}
						/>
						<Route
							path={`/admin/realization_tabs/${record.id}/edit/:id`}
							component={() => <RealizationCategory category={record} />}
						/>
					</React.Fragment>
				))}
			</div>
