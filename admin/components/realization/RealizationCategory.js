...
@withRouter
@observer
export default class RealizationCategory extends React.Component {
	@observable record;
	@observable files;
	@observable id;
	...
	init = async () => {
		...
		if (!isNaN(id)) {
			...
			record.realizationId = categoryId;
			files = new store.model.CategoryRecordFile();
		}
	...
	@action onSave = async record => {
		this.id && this.props.history.push(`/admin/realization_tabs/${this.props.category.id}/list`);
	...
	render() {
		...
		const onDelete = () => {
			this.props.history.push(`/admin/realization_tabs/${this.props.category.id}/list`);
		};
