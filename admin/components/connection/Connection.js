...
import ConnectionCategoryList from './ConnectionCategoryList';
...
export default class Connection extends React.Component {
	...
	@action switchTab = record => {
		this.tab = record.id;
		this.props.history.push(`/admin/connection_tabs/${record.id}/list`);
			...
			<div>
				...
				{this.records.map(record => (
					<React.Fragment key={`route-${record.id}`}>
						<Route
							exact
							path={`/admin/connection_tabs/${record.id}/list`}
							component={() => <ConnectionCategoryList category={record} />}
						/>
						<Route
							path={`/admin/Connection_tabs/${record.id}/edit/:id`}
							component={() => <ConnectionCategory category={record} />}
						/>
					</React.Fragment>
				))}
			</div>
