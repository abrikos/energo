...
@withRouter
@observer
export default class TradeUnionCategoryList extends React.Component {
	@observable records;
	...
	@action init = async () => {
		const records = await store.model.CategoryRecord.find({
			where: {
				tradeunionId: this.props.category.id,
			},
		});
	...
	render() {
		...
		const onRowClick = record => {
			this.props.history.push(`/admin/tradeunion_tabs/${this.props.category.id}/edit/${record.id}`);
		};
			...
			<div>
				<div className="filters">
					<Button
						...
						onClick={() => this.props.history.push(`/admin/tradeunion_tabs/${this.props.category.id}/edit/create`)}
					/>
				</div>
