...
@withRouter
@observer
export default class CouncilEdit extends React.Component {
	@observable record;
	@observable id;
	...
	init = async () => {
		...
		let record;
		if (!isNaN(id)) {
			record = await store.model.Council.findById(id);
			this.id = id;
			this.imagePreview = record.thumbnail ? record.downloadFile('thumbnail') : null;
		} else {
			record = new store.model.Council();
			record.status = false;
		}
		this.record = record;
	...
	@action save = async record => {
		...
		this.id && this.props.history.push('/admin/council');
		// await record.save();
		this.id = record.id;
		// console.log('saved');
	};
