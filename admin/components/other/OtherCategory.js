...
export default class OtherCategory extends React.Component {
	...
	@action onSave = async record => {
		this.id && this.props.history.push(`/admin/other_tabs/${this.props.category.id}/list`);
	...
	render() {
		...
		const onDelete = () => {
			this.props.history.push(`/admin/other_tabs/${this.props.category.id}/list`);
		};
